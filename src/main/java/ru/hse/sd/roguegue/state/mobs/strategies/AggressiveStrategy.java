...
public class AggressiveStrategy extends MobStrategy {
    ...
    public Position getNewPosition(Position position) {
        ...
        if (super.validatePosition(position.move(Move.UP))
                && (position.move(Move.UP).distance(userPosition) < userPosition.distance(position))) {
            return position.move(Move.UP);
        } else if (super.validatePosition(position.move(Move.DOWN))
                && (position.move(Move.DOWN).distance(userPosition) < userPosition.distance(position))) {
            return position.move(Move.DOWN);
        } else if (super.validatePosition(position.move(Move.LEFT))
                && (position.move(Move.LEFT).distance(userPosition) < userPosition.distance(position))) {
            return position.move(Move.LEFT);
        } else if (super.validatePosition(position.move(Move.RIGHT))
                && (position.move(Move.RIGHT).distance(userPosition) < userPosition.distance(position))) {
