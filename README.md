...
#### GameObjectAction
Преобразовывает полученные данные в действие (передвижение, подбор вещей, удар) для конкретного объекта игры, 
в зависимости от которого изменяет необходимые State: GameObjectState, GameState. От него будут наследоваться действия объектов игры (сам персонаж, артефакты, монстры)
